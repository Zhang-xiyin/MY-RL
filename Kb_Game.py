...
class KB_Game:
    ...
    def train(self,play_total,policy,**kwargs):
        ...
        for i in range(play_total):
            ...
            self.counts += 1
            index = self.a-1
            self.q[index] = (self.q[index] * self.action_counts[index]+self.r)/(self.action_counts[index]+1)
...
if __name__ == '__main__':
    ...
    K_gamble = KB_Game()
    total = 2000
    K_gamble.train(play_total=total,policy = 'e_greedy',epsilon = 0.5)
    K_gamble.plot(colors='r',policy='e_greedy',style='-.')
    K_gamble.reset()
    K_gamble.train(play_total=total,policy ='boltzman',temperature = 0.5)
